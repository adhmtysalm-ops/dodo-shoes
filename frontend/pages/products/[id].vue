<template>
  <div dir="rtl" class="font-sans">
    <div v-if="pending" class="py-20 text-center font-bold">جاري التحميل...</div>
    <div v-else-if="error || !product" class="py-20 text-center text-brand-red font-bold">المنتج غير موجود</div>
    ...
    <div v-else class="max-w-7xl mx-auto px-6 py-12 grid grid-cols-1 md:grid-cols-2 gap-12">
      ...
      <div class="flex flex-col justify-center">
        ...
        <p class="text-gray-600 mb-8 max-w-md leading-relaxed">
          {{ product.description || "الرفيق المثالي لجرياتك اليومية. مصمم لتحقيق أقصى درجات السرعة والراحة في كل خطوة." }}
        </p>
...
const { data: product, pending, error } = useFetch(`${config.public.apiUrl}/api/products/${route.params.id}`)
</script>
